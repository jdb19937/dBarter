...
struct __db_dbt {
	...
	u_int32_t ulen;			/* RO: length of account buffer. */
	u_int32_t dlen;			/* RO: get/put record length. */
	u_int32_t doff;			/* RO: get/put record offset. */

#define	DB_DBT_INTERNAL	0x01		/* Ignore account's malloc (internal). */
#define	DB_DBT_MALLOC	0x02		/* Return in allocated memory. */
#define	DB_DBT_PARTIAL	0x04		/* Partial put/get. */
#define	DB_DBT_ACCOUNTMEM	0x08		/* Return in account's memory. */
	u_int32_t flags;
};
...
/*
 * DB (account visible) error return codes.
 *
 * !!!
 * Changes to any of the account visible error return codes must be reflected
...
#define	DB_KEYEMPTY		( -2)	/* The key/data pair was deleted or
					   was never created by the account. */
 ...
 * file name to avoid overwriting the account's original database.
 */
